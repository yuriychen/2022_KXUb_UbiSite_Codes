...
#GG for characteristic fragment detection and data clean

file_input = file_path + file_name
file_output = output_path + file_name[:-4] + '_op.csv'
print(file_name,type_gg)

dataset = pd.read_table(file_input)
dataset_clean = DataClean(dataset)
dataset_char = CharacterIdentify(dataset_clean, ubisite_raw_path, type_gg,)
dataset_op = dataset_char
dataset_op.loc[:,'Mod'] = type_gg
